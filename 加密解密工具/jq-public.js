    ...
    var inputSave = {
        ...
        ToolBoxAdd: function () {
            ...
            if (location.host.indexOf("mobile") >= 0)//如果是mobile.chinaz.com
                $.get('/fit/toobox', { "addval": escape(val), "iswords": iswords });
            else
                $.get('/ajaxsync.aspx?at=toolbox&iswords=' + iswords + '&addval=' + escape(val));
        },
        ...
        ToolBoxDeleteValue: function (val) {
            inputSave.BoxHide();
            if (location.host.indexOf("mobile") >= 0)//如果是mobile.chinaz.com
                $.get('/fit/toobox', { "delval": escape(val), "iswords": iswords });
            else
                $.get('/ajaxsync.aspx?at=toolbox&iswords=' + iswords + '&delval=' + escape(val));
        },
...
var boxScroll = function (options) {
    ...
    function _scrollfn() {
        ...
        if (settings.endElm) {
            ...
            _scroll.css({
                position: _postiton,
                left: (settings._winWidth - settings._width - 30) + "px",
...
/*function validate() {
    var $url = $("input[url=\"true\"]");
    var $words = $("input[words=\"true\"]");
    ...
    var _validate = function (_this, isSub) {
        ...
        if ($words.length) {
            if (isSub) {
                return _checked($words);
            } else {
                _checked($words);
            }
        }
    }
    var _checked = function ($obj) {
        for (var i = 0; i < $obj.length; i++) {
            var $elm = $($obj[i]);
            if (!$elm.val().trim()) {
                $elm.siblings(".CentHid").addClass("col-red");
                return false;
            } else {
                $elm.siblings(".CentHid").removeClass("col-red");
                if (i == $obj.length - 1)
                    return true;
            }
        }
